...
entity ImmMux is
  port(
    -- Input bus ALUSrc signals
    ALUSrc_flg: in T_SYSTEM_BOOL;
    -- Input bus OutputB signals
    OutputB_data: in T_SYSTEM_UINT32;
    -- Input bus SignExtOut signals
    SignExtOut_data: in T_SYSTEM_UINT32;

    -- Output bus ImmMuxOut signals
    ImmMuxOut_data: out T_SYSTEM_UINT32;
...
begin
	...
	begin
        ...
        if RST = '1' then
            ImmMuxOut_data <= STD_LOGIC_VECTOR(TO_UNSIGNED(0, T_SYSTEM_UINT32'length));
            flg := '0';
            ...
            flg := ALUSrc_flg;
            if flg = '1' then
                ImmMuxOut_data <= SignExtOut_data;
            else
                ImmMuxOut_data <= OutputB_data;
            end if;
