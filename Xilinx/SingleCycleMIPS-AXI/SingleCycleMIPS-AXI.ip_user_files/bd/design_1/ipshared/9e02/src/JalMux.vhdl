...
entity JalMux is
  port(
    -- Input bus MuxOutput signals
    MuxOutput_addr: in T_SYSTEM_UINT8;
    -- Input bus JAL signals
    JAL_flg: in T_SYSTEM_BOOL;

    -- Output bus RegWriteAddr signals
    RegWriteAddr_addr: out T_SYSTEM_UINT8;
...
begin
	...
	begin
        ...
        if RST = '1' then
            RegWriteAddr_addr <= STD_LOGIC_VECTOR(TO_UNSIGNED(0, T_SYSTEM_UINT8'length));
            ...
            if JAL_flg = '1' then
                ...
                local_var_0 := MuxOutput_addr;
            end if;
            RegWriteAddr_addr <= local_var_0;
