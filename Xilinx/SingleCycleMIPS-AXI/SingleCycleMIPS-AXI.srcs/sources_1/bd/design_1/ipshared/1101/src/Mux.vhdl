...
entity Mux is
  port(
    -- Input bus RegDst signals
    RegDst_flg: in T_SYSTEM_BOOL;
    -- Input bus MuxInput signals
    MuxInput_rt: in T_SYSTEM_UINT8;
    MuxInput_rd: in T_SYSTEM_UINT8;

    -- Output bus MuxOutput signals
    MuxOutput_addr: out T_SYSTEM_UINT8;
...
begin
	...
	begin
        ...
        if RST = '1' then
            MuxOutput_addr <= STD_LOGIC_VECTOR(TO_UNSIGNED(0, T_SYSTEM_UINT8'length));
            ...
            if RegDst_flg = '1' then
                local_var_0 := MuxInput_rd;
            else
                local_var_0 := MuxInput_rt;
            end if;
            MuxOutput_addr <= local_var_0;
