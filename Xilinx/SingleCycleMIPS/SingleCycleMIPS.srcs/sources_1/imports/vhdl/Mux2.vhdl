...
entity Mux2 is
  port(
    ...
    -- Input bus ShiftJump signals
    ShiftJump_addr: in T_SYSTEM_UINT32;
    -- Input bus JumpReg signals
    JumpReg_flg: in T_SYSTEM_BOOL;

    -- Output bus Mux2Out signals
    Mux2Out_addr: out T_SYSTEM_UINT32;
...
begin
	...
	begin
        ...
        if RST = '1' then
            Mux2Out_addr <= STD_LOGIC_VECTOR(TO_UNSIGNED(0, T_SYSTEM_UINT32'length));
            ...
            if JumpReg_flg = '1' then
                local_var_0 := OutputA_data;
            else
                local_var_0 := ShiftJump_addr;
            end if;
            Mux2Out_addr <= local_var_0;
