...
entity EX_JumpJrMux is
  port(
    -- Input bus EX_ForwardA signals
    EX_ForwardA_data: in T_SYSTEM_UINT32;
    -- Input bus EX_JumpPacked signals
    EX_JumpPacked_addr: in T_SYSTEM_UINT32;
    -- Input bus EX_JumpReg signals
    EX_JumpReg_flg: in T_SYSTEM_BOOL;

    -- Output bus EX_JumpAddress signals
    EX_JumpAddress_addr: out T_SYSTEM_UINT32;
...
architecture RTL of EX_JumpJrMux is
...
begin
	...
	begin
        ...
        if RST = '1' then
            EX_JumpAddress_addr <= STD_LOGIC_VECTOR(TO_UNSIGNED(0, T_SYSTEM_UINT32'length));
            ...
            if EX_JumpReg_flg = '1' then
                local_var_0 := EX_ForwardA_data;
            else
                local_var_0 := EX_JumpPacked_addr;
            end if;
            EX_JumpAddress_addr <= local_var_0;
